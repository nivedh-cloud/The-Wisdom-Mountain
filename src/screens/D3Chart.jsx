...
export default function D3Chart({ lang = 'en' }) {
  console.log('🎯 D3Chart component rendered with lang:', lang);
  ...
  const createInlineTooltip = (event, d) => {
    console.log('🔥 INLINE TOOLTIP: Current language:', lang);
    ...
    if (d.data.class) {
      ...
      console.log('🔥 INLINE TOOLTIP: lang value:', lang, 'classLabel:', classLabel);
  ...
  const createDetailedTooltip = useCallback((event, d) => {
    console.log('🔥 TOOLTIP: Creating tooltip for:', d.data.name);
    console.log('🔥 TOOLTIP: Current language in tooltip function:', lang);
    console.log('🔥 TOOLTIP: Available fields:', {
      name: d.data.name,
      nameEn: d.data.nameEn,
      nameTe: d.data.nameTe,
      spouseEn: d.data.spouseEn,
      spouseTe: d.data.spouseTe,
      detailEn: d.data.detailEn,
      detailTe: d.data.detailTe,
      children: d.data.children ? d.data.children.length : 0,
      childrenNames: d.data.children_names
    });
    ...
    console.log('Tooltip fields:', { tooltipName, tooltipSpouse, tooltipDetail });
    ...
    if (d.data.class) {
      const classLabel = lang === 'te' ? 'వర్గము:' : 'Class:';
      console.log('🔥 TOOLTIP: lang value:', lang, 'classLabel:', classLabel);
      content += `<br/><span style="color: #34d399;">${classLabel}</span> ${d.data.class}`;
    }
  ...
  useEffect(() => {
    console.log('🔄 D3Chart useEffect triggered, lang:', lang, 'isVertical:', isVertical, 'expandAll:', expandAll);
    ...
    console.log('Selected bilingual data for language', lang, ':', selectedData); // Debug log
    console.log('Hierarchy data:', hierarchyData); // Debug log
    ...
    const toggleNode = (nodeData) => {
      console.log('Toggling node:', nodeData.name);
      ...
      updateNodeInHierarchy(hierarchyData, nodeData.name, (targetNode) => {
        if (targetNode._children && targetNode._children.length > 0) {
          ...
          console.log('Expanded node:', targetNode.name, 'now has', targetNode.children.length, 'visible children');
          ...
          console.log('Collapsed node:', targetNode.name, 'now has', targetNode._children.length, 'hidden children');
        }
      });
    ...
    console.log('D3 root:', root); // Debug log
    ...
    console.log('Links created:', links.size()); // Debug log
    ...
    console.log('Node groups created:', nodeGroups.size()); // Debug log
    ...
    console.log('Shapes created for nodes based on gender'); // Debug log
    ...
    const textLabels = nodeGroups.append("text")
      ...
      .text(d => {
        ...
        if (d.data.name === 'Adam' || d.data.nameEn === 'Adam' || d.data.name === 'Seth' || 
          ...
          console.log('🖼️ Rendering text for node:', {
            searchName: d.data.name,
    ...
    console.log('Text labels with backgrounds created for nodes'); // Debug log
    ...
    nodeGroups.each(function(d) {
      ...
      if (hasHiddenChildren && !hasVisibleChildren) {
        // Show expand icon
        console.log('Node:', d.data.name, 'showing expand icon (+)');
        ...
        console.log('Node:', d.data.name, 'showing collapse icon (-)');
        ...
        console.log('Node:', d.data.name, 'no icon (original children or no expandable content)');
      }
    });

    console.log('Expand/collapse buttons added for nodes with _children'); // Debug log
    }; // End of createTreeElements function

    // Initial tree creation
    const treeData = treeLayout(root);
    console.log('Tree data:', treeData); // Debug log
    console.log('Descendants count:', treeData.descendants().length); // Debug log
    console.log('Links count:', treeData.links().length); // Debug log
    ...
    console.log('Links created'); // Debug log
    
    // Debug: List all available nodes in the chart for search debugging
    setTimeout(() => {
      const svg = d3.select(svgRef.current);
      const nodes = svg.selectAll('.node');
      console.log('=== Available nodes in chart for search ===');
      nodes.each(function(d, i) {
        console.log(`${i}: name="${d.data.name}", name_en="${d.data.name_en}", name_te="${d.data.name_te}"`);
      });
      console.log('=== End of available nodes ===');
  ...
  useEffect(() => {
    ...
    console.log('🔄 Updated text labels for language:', lang);
    ...
    if (svgRef.current) {
      try {
        ...
        if (currentTransform && (currentTransform.k !== 1 || currentTransform.x !== 0 || currentTransform.y !== 0)) {
          console.log('💾 Saving transform before expand/collapse change:', currentTransform);
          setSavedTransform(currentTransform);
        }
      } catch (error) {
        console.log('Error getting current transform:', error);
      }
    }
    ...
    if ((savedTransform || savedTransformForLineStyle) && svgRef.current) {
      ...
      const attemptRestore = () => {
        ...
        if (zoomBehavior && hasContent) {
          ...
          console.log('🔄 Restoring transform:', transformToRestore);
          
          try {
            svg.call(zoomBehavior.transform, transformToRestore);
            console.log('✅ Transform restored successfully');
            ...
            console.log('❌ Transform restoration error:', error);
  ...
  useEffect(() => {
    ...
    console.log('Theme changed, updating text colors and backgrounds. isDarkMode:', isDarkMode);
  ...
  const convertToHierarchy = (data, language = 'en') => {
    console.log('🔄 convertToHierarchy called with language:', language, 'expandAll:', expandAll);
    ...
    const addTranslatedNames = (node, currentDepth = 0, maxDepth = 6) => {
      ...
      if (currentDepth < maxDepth && node._children && Array.isArray(node._children) && node._children.length > 0) {
        console.log(`Node ${node.name} has _children, expandAll is:`, expandAll);
        if (expandAll) {
          ...
          console.log(`Expanded node ${displayName} - added ${expandedChildren.length} children`);
        } else {
          ...
          console.log(`Collapsed node ${displayName} - kept ${result._children.length} children hidden`);
        }
      }
  ...
  const expandAllNodes = () => {
    console.log('expandAllNodes called, current expandAll:', expandAll);
    ...
    console.log('Expanding all nodes - expandAll set to true');
  };

  // Function to collapse all user-expanded nodes  
  const collapseAllNodes = () => {
    console.log('collapseAllNodes called, current expandAll:', expandAll);
    ...
    setExpandAll(false);
    setForceUpdate(prev => prev + 1); // Force re-render
    console.log('Collapsing all nodes - expandAll set to false');
  };

  // Function to toggle expand/collapse all
  const toggleExpandCollapseAll = () => {
    console.log('toggleExpandCollapseAll clicked! Current expandAll:', expandAll);
    if (expandAll) {
      console.log('Calling collapseAllNodes');
      collapseAllNodes();
    } else {
      console.log('Calling expandAllNodes');
      expandAllNodes();
    }
  };
  ...
  const handleSearch = (event) => {
    ...
    if (term.length >= 2) {
      ...
      const searchInBilingualData = () => {
        const searchInNode = (node) => {
          ...
          if (searchName.includes(searchTerm)) {
            console.log('Match found:', { searchName: node.name, nameEn: node.nameEn, nameTe: node.nameTe });
      ...
      console.log('Search result names for debugging:');
      results.forEach((result, index) => {
        console.log(`${index}: Display="${result.name}", Original="${result.originalName}", EN="${result.nameEn}", TE="${result.nameTe}"`);
      });
  ...
  const zoomToPerson = (person) => {
    try {
      console.log('Zooming to person:', person);
      const svg = d3.select(svgRef.current);
      const zoom = svg.node()?.zoomBehavior;
      
      if (!zoom) {
        console.log('Zoom behavior not found');
        return;
      }
      ...
      console.log('Looking for target name:', targetName);
      ...
      nodes.each(function(d) {
        console.log('Checking node:', d.data.name, 'vs target:', targetName);
        ...
        if (!isMatch && originalNode) {
          if ((d.data.nameEn && originalNode.nameEn && 
            ...
            isMatch = true;
            console.log('Bilingual name match found');
          }
        }
        ...
        if (!isMatch && d.data && targetName) {
          ...
          for (const field of checkFields) {
            if (field && field.toLowerCase().includes(targetName.toLowerCase())) {
              isMatch = true;
              console.log('Contains match found in field:', field);
              break;
            }
          }
        }
        ...
        if (isMatch) {
          targetNode = d;
          console.log('Found target node:', d);
      ...
      if (targetNode) {
        ...
        console.log('Transform values:', translateX, translateY, scale);
        ...
        console.log('Zoomed to target successfully');
        ...
        console.log('Target node not found for person:', targetName);
      ...
      console.error('Error in zoomToPerson:', error);
  ...
  const zoomToNodeOnClick = (nodeData) => {
    console.log('Zooming to clicked node:', nodeData);
    const svg = d3.select(svgRef.current);
    const zoom = svg.node().zoomBehavior;
    ...
    nodes.each(function(d) {
      if (d.data.name === nodeData.name) {
        targetNode = d;
        console.log('Found target node for click:', d);
      }
    });
    ...
    if (targetNode) {
      ...
      console.log('Node click - Container dimensions:', containerRect.width, 'x', containerRect.height);
      console.log('Node click - Target coordinates:', targetX, targetY);
      console.log('Node click - Center coordinates:', centerX, centerY);
      ...
      console.log('Node click - Transform values:', translateX, translateY, scale);
      ...
      console.log('Zoomed to clicked node successfully');
    } else {
      console.log('Target node not found for clicked node:', nodeData.name);
    }
  };
